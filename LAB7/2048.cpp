...
public:
	...
	virtual float estimate(const board& b) const {
        ...
        for (int i = 0; i < iso_last; i ++) {
			...
			for (int j = 0; j < isomorphic[i].size(); j ++) {
			    tuple_sum |= b.at(isomorphic[i][j]) << (j * 4);
 			}
			board_value += operator[](tuple_sum);
		}
	...
	virtual float update(const board& b, float u) {
		...
		for(int i = 0; i < iso_last; i ++) {
			...
			operator[](tuple_sum) += u_split;
			value += operator[](tuple_sum);
		}
        return value;
	}
...
protected:
	...
	// Get the index in the n-tuple tree
...
public:
	...
	bool assign(const board& b) {
		...
		after = before = b;
		score = after.move(opcode); // score is the reward of after state
		esti = score;
		return score != -1;
	}
